...
def measure_insertion_time_and_disk_space(file_path):
    ...
    with open(file_path, 'rb') as file:
        ...
        while True:
            ...
            if not data:
                break
            cursor.execute("INSERT INTO tabela_arquivo3 (content) VALUES (%s)", (data,))

    conn.commit()
    cursor.close()
    conn.close()
    print("Arquivo 3 inserido com sucesso no banco de dados!\n")
    ...
    # Excluir o arquivo
    os.remove(file_path)
    print("Arquivo 3 excluído com sucesso! (Para liberar espaço no disco)\n")
...
def delete_table():
    ...
    cursor.execute("DROP TABLE IF EXISTS tabela_arquivo3")
...
# Gera um arquivo com registros longos (110 KB) e muitos registros (1 milhão)
def generate_long_records_file(file_path, num_records):
    ...
    print("Arquivo 3 gerado com sucesso!\n")
